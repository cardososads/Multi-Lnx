<div id="defender-totp" class="<?php echo esc_attr( $class ); ?>">
	<div class="card">
		<strong>
			<?php _e( '1. Install the Verification app', 'defender-security' ); ?>
		</strong>
		<p>
			<?php
			_e( 'Download and install the authenticator app on your device using the links below.', 'defender-security' );
			?>
			<select id="auth-app">
				<?php
		...
		<div class="line"></div>
		<strong>
			<?php _e( '2. Scan the QR code or enter the key', 'defender-security' ); ?>
		</strong>
		<?php if ( $is_success ) { ?>
		...
		<div class="line"></div>
		<strong>
			<?php _e( '3. Enter passcode', 'defender-security' ); ?>
		</strong>
		<p>
			<?php
			_e(
		...
		<div class="well" style="width: auto;">
			...
			<input type="text" id="otp-code" class="def-small-text"
			       placeholder="<?php _e( 'Enter passcode', 'defender-security' ); ?>" />
