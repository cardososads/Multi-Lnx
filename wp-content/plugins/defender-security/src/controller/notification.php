	...
	public function show_actions_with_subscription() {
		...
		if ( 'subscribed' === $context ) {
			...
			/* translators: %s - module title, %s - unsubscribed link */
				__( 'You are now subscribed to receive <strong>%1$s</strong>. Made a mistake? <a href="%2$s">Unsubscribe</a>', 'defender-security' ),
				$m->title,
				$unsubscribe_link
			);
			...
			/* translators: %s - module title */
				__( 'You are now unsubscribed from <strong>%s</strong>.', 'defender-security' ),
				$m->title
			);
		} else {
			return null;
	...
	public function validate_email( Request $request ) {
		...
		if ( filter_var( $email, FILTER_VALIDATE_EMAIL ) ) {
			...
			return new Response(
				false,
				array(
					'error' => __( 'Invalid email address.', 'defender-security' ),
				)
			);
		}
	}
	...
	public function resend_invite_email( Request $request ) {
		...
		if ( ! is_object( $model ) ) {
			return new Response( false, [
				'message' => __( 'Module not found.', 'defender-security' ),
			] );
		}
		...
		if ( $sent ) {
			return new Response( true, [
				'message' => __( 'Invitation sent successfully.', 'defender-security' ),
			] );
		}
