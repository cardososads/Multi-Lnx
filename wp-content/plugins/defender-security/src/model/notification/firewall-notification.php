	...
	private function send_to_user( string $email, string $name, Lockout_Log $model, string $template, object $service ): void {
		...
		if ( 'login-lockout' === $template ) {
			...
			if ( 'permanent' === $settings->lockout_type ) {
				...
				$string = sprintf( __( 'They have been locked out for <strong>%1$s.</strong>', 'defender-security' ), $time_limit );// phpcs:ignore
			}
			$text = sprintf(
				$text,
				$model->ip,
			...
			if ( 'permanent' === $settings->lockout_type ) {
				...
				$string = sprintf( __( 'They have been locked out for <strong>%1$s %2$s.</strong>', 'defender-security' ), $settings->duration, $settings->duration_unit );// phpcs:ignore
			}
			$text = sprintf(
				$text,
				$model->ip,
