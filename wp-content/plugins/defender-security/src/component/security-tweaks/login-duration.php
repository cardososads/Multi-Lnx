...
class Login_Duration extends Component {
	...
	public function process() {
		...
		if ( $this->is_incorect_duration( $duration ) ) {
			...
			return new WP_Error(
				...
				__( 'Duration can only be a number and greater than 0.', 'defender-security' )
			);
		}
		$this->resolved = true;
	...
	public function to_array(): array {
		...
		return [
			'slug' => $this->slug,
			'title' => __( 'Manage Login Duration', 'defender-security' ),
			'errorReason' => sprintf(
				__( 'Your current login duration is the default %d days.', 'defender-security' ),
				self::DEFAULT_DAYS
			),
			'successReason' => sprintf(
				__( 'You\'ve adjusted the default login duration to %d days.', 'defender-security' ),
				$duration
			),
			'misc' => [ 'duration' => $duration ],
			'bulk_description' => __( 'Users who select the \'remember me\' option will stay logged in for 14 days.It’s good practice to reduce this default time to reduce the risk of someone gaining access to your automatically logged in account. We’ll set the login duration to 7 days.', 'defender-security' ),
			'bulk_title' => __( 'Login Duration', 'defender-security' ),
		];
	}
}
