	...
	public function verify_attempt( int $user_id, string $slug ): string {
		...
		*/
		if (
			...
			/* translators: %s: count */
				_n( 'You have %s login attempt remaining.', 'You have %s login attempts remaining.', $count, 'defender-security' ),
				$count
			);
		...
		<div class="notice notice-success is-dismissible">
			<p>
				...
				/* translators: %s: URL to regenerate code */
					__( 'Two factor authentication has been reset for <b>%s.</b>', 'defender-security' ),
					$user->display_name
				);
				?>
			</p>
