...
class Menu_Audit extends Audit_Event {
		...
		if ( ! empty( $old_menu ) ) {
						...
						if (
							...
							return [
								...
								/* translators: 1: Blog name, 2: User's display name, 3: Menu item, 4: Menu name */
									__( '%1$s %2$s updated item "%3$s" from menu "%4$s"', 'defender-security' ),
									$blog_name,
			...
			if ( in_array( $menu_item_db_id, $added_items, true ) ) {
				return [
					sprintf(
					/* translators: 1: Blog name, 2: User's display name, 3: Menu item, 4: Menu name */
						__( '%1$s %2$s added item "%3$s" to menu "%4$s"', 'defender-security' ),
						$blog_name,
						$this->get_user_display( get_current_user_id() ),
						$post_array['menu-item-title'][ $menu_item_db_id ],
						$post_array['menu-name']
		...
		if ( ! empty( $old_menu ) && is_array( $old_menu ) ) {
			...
			if ( in_array( $menu_item_db_id, $old_items, true ) ) {
				...
				return [
					sprintf(
					/* translators: 1: Blog name, 2: User's display name, 3: Menu item, 4: Menu name */
						__( '%1$s %2$s removed item "%3$s" from menu "%4$s"', 'defender-security' ),
						$blog_name,
						$this->get_user_display( get_current_user_id() ),
	...
	public function menu_created_callback(): array {
		...
		return [
			...
			/* translators: 1: Blog name, 2: User's display name, 3: Menu name */
				__( '%1$s %2$s created a new menu "%3$s"', 'defender-security' ),
				$blog_name,
				$this->get_user_display( get_current_user_id() ),
				$menu_data['menu-name']
			),
		...
		if ( $new_items === $old_items ) {
			...
			if ( false === $is_any_item_changed ) {
				...
				return [
					sprintf(
					/* translators: 1: Blog name, 2: User's display name, 3: Menu name */
						__( '%1$s %2$s updated menu "%3$s"', 'defender-security' ),
						$blog_name,
						$this->get_user_display( get_current_user_id() ),
						$menu_data['menu-name']
					),
	...
	public function menu_deleted_callback(): array {
		...
		return [
			sprintf(
			/* translators: 1: Blog name, 2: User's display name, 3: Menu name */
				__( '%1$s %2$s deleted menu "%3$s"', 'defender-security' ),
				$blog_name,
				$this->get_user_display( get_current_user_id() ),
				$old_menu['name']
			),
