	...
	*/
	...
	private static function decrypt( $encoded_value, $key ) {
		if ( ! $encoded_value || '' === $key ) {
			return new WP_Error(
				...
				__( 'Please re-setup 2FA TOTP method again.', 'defender-security' )
			);
		}
		$key = base64_decode( $key );
	...
	private static function get_random_key() {
		...
		if ( ! file_exists( $file ) ) {
			return new WP_Error(
				Error_Code::IS_EMPTY,
				__( 'The Defender file with the random key does not exist.', 'defender-security' )
			);
		}
		...
		if ( '{{__REPLACE_CODE__}}' !== constant( 'WP_DEFENDER_TOTP_KEY' ) ) {
			...
			return new WP_Error( Error_Code::INVALID, __( 'The Defender file with the random key is incorrect.', 'defender-security' ) );
		}
	}
