...
class Webauthn extends Two_Factor_Provider {
	...
	public function get_user_label(): string {
		return __( 'Web Authentication', 'defender-security' );
	}
	...
	public function get_description(): string {
		return __( "Authenticate login using your device's built-in fingerprint, facial recognition, or an external hardware security key.", 'defender-security' );
	}
	...
	public function user_options( WP_User $user ): void {
		$notices = [
			[
				...
				'style' => 'display: none',
				'message' => __( 'Access from unregistered devices will be denied. To avoid this, consider setting up an additional authentication method.', 'defender-security' ),
			],
		];
		...
		if ( $user_has_admin_capability ) {
			...
			$notice_message = __( 'Your site doesn\'t meet the web authentication requirement. Please contact your site\'s administrator to fix this issue.', 'defender-security' );
		}
	...
	public function authentication_form() {
		...
		<div class="welcome-screen">
			<p class="wpdef-2fa-label"><?php esc_html_e( 'Web Authentication', 'defender-security' ); ?></p>
		...
		<div class="webauthn-content-wrap">
			<div class="webauthn-platform" style="display:none;">
				...
				<p class="wpdef-2fa-text">
					<?php esc_html_e( 'Please verify your identity using fingerprint or facial recognition.', 'defender-security' ); ?>
				</p>
			</div>
			...
			<div class="wpdef-2fa-webauthn-control" style="display:none;">
				<button type="button" class="button" id="wpdef-2fa-webauthn-back-btn"><?php esc_html_e( 'Back', 'defender-security' ); ?></button>
	...
	private function missed_requirement_message(): array {
		...
		if ( ! $this->is_ssl() ) {
			...
			$warning_core_message .= __( 'SSL certificate or HTTPS is not forced. ', 'defender-security' );
		}
	...
	private function concatenate_failed_extensions( array $extensions ): string {
		...
		if ( count( $extensions ) > 1 ) {
			$last_extension = __( ' and ', 'defender-security' ) . array_pop( $extensions );
