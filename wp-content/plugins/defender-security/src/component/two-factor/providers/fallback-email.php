...
class Fallback_Email extends Two_Factor_Provider {
	...
	public function get_label(): string {
		return __( 'Fallback Email', 'defender-security' );
	}
	...
	public function get_description(): string {
		return __( 'If you ever lose your device, you can send a fallback passcode to this email address.', 'defender-security' );
	}
	...
	public function authentication_form() {
		...
		<p class="wpdef-2fa-text">
			<?php echo __( "Can't access your device? We've sent a passcode to your fallback email address. Enter it below to log in to your account.", 'defender-security' ); ?>
		</p>
		...
		<p class="wpdef-2fa-email-resend">
			<input type="submit" class="button" name="button_resend_code" value="<?php esc_attr_e( 'Resend Code', 'defender-security' ); ?>" />
		</p>
		<?php
	}
	...
	public function validate_authentication( $user ) {
		...
		if ( ! empty( $backup_code ) && Crypt::compare_lines( $backup_code['code'], wp_hash( $otp ) )
			...
			return new WP_Error(
				'opt_fail',
				empty( $lockout_message ) ? __( 'ERROR: Invalid passcode.', 'defender-security' ) : $lockout_message
			);
		}
	}
}
