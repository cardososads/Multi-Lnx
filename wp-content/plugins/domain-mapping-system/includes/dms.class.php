...
class DMS
{
    ...
    public function getData( $item_per_page = 200, $page = null, $use_page_query = false )
    {
        ...
        if ( !empty($ids) ) {
            ...
            $ids_str = implode( ',', $ids );
            // Retrieve full data
            return $wpdb->get_results( "SELECT m.id, m.host, m.path, m.main, m.`attachment_id`, m.`order`, mv.value, mv.primary FROM `" . $wpdb->prefix . "dms_mappings` m \n            LEFT JOIN `" . $wpdb->prefix . "dms_mapping_values` mv ON m.id=mv.host_id\n            WHERE m.id IN (" . $ids_str . ") ORDER BY m.`order`, mv.order\n        " );
        }
    ...
    public static function prepareUriFilters()
    {
        ...
        if ( !is_admin() ) {
            ...
            // The below ones are related to any (almost) kind of links in the page
            //			add_filter( 'paginate_links', array( $DMS, 'rewritePaginationLinks' ), 99 );
            //			add_filter( 'nav_menu_link_attributes', array( $DMS, 'rewriteNavMenuLink' ), 99,
            //				4 ); // Possible other workaround is "wp_nav_menu_objects" filter
            // Bellow line could be omitted in case home_url filter exists. Because otherwise the page links would change twice
            // and this example.com -> something-example.com mapping case would take bad effect, by turning to something-something-example.com
        ...
        /* TODO
         * List of hooks related with archives
         *   day_link
         *   month_link
         *   author_link
         *   year_link
         * List of hooks related with feeds
         *   feed_link
         *   self_link
         *   author_feed_link
         * Yoast
         ...
         *   wpseo_sitemap_entry
         * Elementor
         *   elementor/document/urls/preview (currently solved with simple condition - (!isset($_GET['elementor-preview'])) in redirects catch function )
         */
        //		 add_filter( 'elementor/document/urls/preview', array($DMS, 'replaceElementorPreviewUrl') );
    }
